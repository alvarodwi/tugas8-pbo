...
class SlipGaji:
    ...
    def hitungGaji(self):
        if(self.__waktuMasuk.getJam() < 8 and self.__waktuPulang.getJam() > 16):
            self.__lamaKerja.setJam(8)
        elif (self.__waktuMasuk.getJam() < 8 and self.__waktuPulang.getJam() < 16):
            ...
            self.__lamaKerja = self.__waktuMasuk.hitungSelisih(self.__waktuPulang)
        self.__gaji = self.__lamaKerja.getJam() * 50000

    def printSlipGaji(self):
        print(".:: SLIP GAJI ::.")
        print("KodePegawai\t: " + str(self.__kodePegawai))
        print("Nama\t\t: " + self.__namaPegawai)
